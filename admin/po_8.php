...
$t = date("h:i:sa");
    ...
    if($con){ 
        ...
        $sql = "INSERT INTO `notifications`(`id`, `panel`, `message`, `status`, `date`, `time`, `sender_panel`, `sender_id`,`po_id`)
                VALUES (Null,'Accounts','Check Payment Recived From Dealer','Active','$d','$t','Operations','$panel_id','$po_id')";

        $sql2 = "INSERT INTO `notifications_entry`(`id`, `n_id`, `date`, `time`, `po_id`) VALUES (Null,'$panel_id','$d','$t','$po_id')";
        
        $sql3 = "UPDATE `po` SET `company_status` = 'Accounts',
        `dept_status`='Check_payment'
         WHERE po_id = '$po_id'";
