...
if($con){ 
    ...
    $q = "SELECT from `model_inv` WHERE  `dealer_id` ='$d_id' AND  `model` = '$model'";
    ...
    // $m_count = $m - '1';
  echo json_encode(['status'=>true,'message'=> $m]);
        // $sql="UPDATE `inventory` SET `$varient`='$m_count' WHERE dealer_id = '$d_id' ";
        //  $result = mysqli_query($con,$sql);

        //  $sql="UPDATE `model_inv` SET `$inventory`='$m_count' WHERE dealer_id = '$d_id' AND model = '$model'  ";
        //  $result = mysqli_query($con,$sql);
    ...
    // VALUES ('null','$invoice_id','".$_POST['d_id']."','".$_POST['c_name']."','".$_POST['c_mobile']."','".$_POST['a_mobile']."','".$_POST['email']."','".$_POST['location']."','".$_POST['model_name']."','".$_POST['color']."',
